...
public final class DDSReader
{
    ...
    private static int[] decodeDXT5(int width, int height, int offset, byte[] buffer, Order order)
    {
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            {
                ...
                int b0 =
                    (buffer[index] & 0xFF) | (buffer[index + 1] & 0xFF) << 8 | (buffer[index + 2] & 0xFF) << 16;
                index += 3;
                int b1 =
                    (buffer[index] & 0xFF) | (buffer[index + 1] & 0xFF) << 8 | (buffer[index + 2] & 0xFF) << 16;
                index += 3;
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    int t3 = (buffer[index++] & 0xC0) >> 6;
                    pixels[4 * width * i + 4 * j + width * k] = getDXTColor(c0, c1,
                        getDXT5Alpha(a0, a1, alphaTable[4 * k]), t0, order);
                    if (4 * j + 1 >= width) continue;
                    pixels[4 * width * i + 4 * j + width * k + 1] = getDXTColor(c0, c1,
    ...
    private static int getDXTColor(int c0, int c1, int a, int t, Order order)
    {
        switch (t)
        {
            ...
            case 2:
                return (c0 > c1) ? getDXTColor2_1(c0, c1, a, order) : getDXTColor1_1(c0, c1, a,
                    order);
            case 3:
    ...
    // BIT4 = 17 * index;
    private static final int[] BIT5 = { 0, 8, 16, 25, 33, 41, 49, 58, 66, 74, 82, 90, 99, 107,
        115, 123, 132, 140, 148, 156, 165, 173, 181, 189, 197, 206, 214, 222, 230, 239, 247, 255 };
    private static final int[] BIT6 = { 0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 45, 49, 53, 57,
        61, 65, 69, 73, 77, 81, 85, 89, 93, 97, 101, 105, 109, 113, 117, 121, 125, 130, 134, 138,
        142, 146, 150, 154, 158, 162, 166, 170, 174, 178, 182, 186, 190, 194, 198, 202, 206, 210,
